...
if(window.XMLHttpRequest){
	...
	req = new ActiveXObject("Mxml2.XMLHTTP");   //IE
}



function goUpdate(a,b,c){
 ...
 req.onreadystatechange = function(){
	 if(req.readyState == 4){
		if(req.status==200){
			...
			var a;
			var state="#stateChange"+c;
			if(result==1)
				$(state).html("<span class='label label-default '>预购</span>"); 
			else if(result==2)
				$(state).html("<span class='label label-success'>购入</span>"); 
			else if(result==3)
				$(state).html("<span class='label label-warning'>断货</span>"); 
			else if(result==4)
				$(state).html("<span class='label label-info '>删除</span>"); 
 ...
 req.open("post", "updateState.do" )
req.setRequestHeader("Content-Type", "application/x-www-form-urlencoded"); 
req.send("order_no="+a+"&p_state="+b);  
